...
class Model(pl.LightningModule):
    def __init__(self, config: dict) -> None:
        ...
        self.data_shape = self.config.get("shape",(897, 976))
        ...
        self.noise_pct = self.config.get("noise_pct",0.002)
        ...
        for i in range(self.num_layers,0,-1):
            ...
            func = nn.ConvTranspose2d if i <= self.num_layers-4 else nn.Conv2d
            module_init = func(
                in_channels = in_channels,
                out_channels = out_channels,
                kernel_size = 3,
    ...
    def infer_vae(self,x: torch.tensor) -> torch.tensor:
        ...
        y_hat = self.ckptfn(self.classifier,latents) + 1e-8
        # Process latents for quantization
        B,C,H,W = latents.size()
        ze = latents.permute(0,2,3,1).reshape(-1,C).contiguous()
        # Quantizing latents
        zq = self.quantize(ze)
        ...
        quantized_latents = zq.reshape(B,H,W,C).permute(0,3,1,2).contiguous()
        z_mu = z_mu.reshape(B,H,W,C).permute(0,3,1,2).contiguous()
        z_sigma = z_sigma.reshape(B,H,W,C).permute(0,3,1,2).contiguous()
        ...
        _y_hat = i*v + 1e-8
        z = z.permute(0,2,3,1).reshape(-1,C).contiguous()
        _y_hat = _y_hat.repeat(z.size()[0]//_y_hat.size()[0],1)
        _y_hat = torch.concat((z,_y_hat),dim=1)
        z_bar = self.embed_y(_y_hat).reshape(B,H,W,C).permute(0,3,1,2).contiguous()
        ...
        # reconstructed image
        y_hat_recon = self.ckptfn(self.classifier,latents_recon) + 1e-8
    ...
    def forward(self,x: torch.Tensor, y: torch.Tensor ,batch_idx: int):
        ...
        fake_pred = self.ckptfn(self.discriminator_classifier,fake_pred).contiguous() + 1e-8

        with torch.no_grad():
            fake_pred_gan = self.ckptfn(self.discriminator,fake_pred_tmp)
            fake_pred_gan = self.ckptfn(self.discriminator_classifier,fake_pred_gan).contiguous() + 1e-8

        real_pred = self.ckptfn(self.discriminator,real_pred)
        real_pred = self.ckptfn(self.discriminator_classifier,real_pred).contiguous() + 1e-8
    ...
    def training_step(self, batch: tuple[torch.Tensor], batch_idx: int):
        ...
        x = x + torch.nan_to_num(torch.randn_like(x,device=x.device)*self.noise_pct*(1-sf))
...
root_dir = os.path.join(os.curdir,"SPECT_MPI_Dataset","") # ./dataset/
