...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch: tuple[torch.Tensor], batch_idx: int):
        ...
        self.log('train_loss', cl, on_epoch=True, prog_bar=True, logger=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_conf', conf, on_epoch=True, prog_bar=True, logger=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log('test_conf', conf, on_epoch=True, prog_bar=True, logger=True)
...
for i in range(len(test_dataset)):
    ...
    with torch.no_grad():
        # tmp = model.infer_vae(x)  # Use the custom inference method
        y_hat = model.infer_vae(x.unsqueeze(0))[1]  # Extract the tensor containing predictions from the output tuple
        y_probs = F.softmax(y_hat,dim=-1)
