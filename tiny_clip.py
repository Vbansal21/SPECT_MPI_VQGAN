...
class Model(pl.LightningModule):
    def __init__(self,config: dict) -> None:
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.embed_dim*1,self.embed_dim),
            nn.Tanh(),
    ...
    def forward(self, batch: tuple[torch.Tensor], batch_idx: int = None) -> tuple[torch.Tensor]:
        ...
        inp = x
        reshaped = 0
        # inp = F.interpolate(inp,(self.img_dim*4,self.img_dim*4))
        # inp = self.encoder(inp)
        inp = F.interpolate(inp,(self.img_dim,self.img_dim))
        inp = inp.reshape(-1,3,self.img_dim,self.img_dim)
        out = self.vism(inp)
        logits,pooled = out["last_hidden_state"], out["pooler_output"]
        out = self.classifier(pooled.reshape(B,-1))
        return out
